...
- **Funding landed.** The IBKR account is funded and Flex is live: `fetch_log` green since 2026-06-21, first real fills 2026-06-26 (TSLA 2 sh + SPCX 1 sh, both auto-classified `dca_buy`), so `daily.yml` now hard-fails Flex — the unfunded-era `continue-on-error` soft-fail is retired (Law 7). **Open flag:** `positions_snapshot` has 0 rows despite holdings since 06-26 — the Flex query's Open Positions section isn't flowing (IBKR portal side). It must flow before the first sleeve entry: the `sleeve_shares` derivation needs positions market_value + cash.
