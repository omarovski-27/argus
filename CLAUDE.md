...
**Current state (as of 2026-07-07 — verify against `git log`, not this prose):**
...
- **Phase 5 (analyst) — COMPLETE end to end (P1–P4, completion run 2026-07-06/07).** SEC company-facts mapper (12 concepts incl. capex + D&A, `ingestion/sec_facts.py`, multi-issuer with a symbol↔CIK pairing gate) → `fundamentals`/`fundamentals_latest`; split ledger + filed-date split reads (`quant/splits.py`); read-time metrics (`quant/metrics.py`). **Data-pack layer (`analyst/`):** ticker→CIK (`cik.py`), peer discovery (config `peer_overrides` > Finnhub), EDGAR 10-K/DEF 14A section extraction (`filings.py`), yfinance estimates (`estimates.py`), self-healing any-ticker ingestion (`ingest.py`, target AND peers), frozen-pack builder (`data_pack.py`; reduced-depth shape for non-SEC filers — `_filings_health` guards the string-note shape). **Valuation engine (`quant/valuation.py`):** 3-scenario discounted range, sensitivity, base-rate flag, bear-weighted MoS, reverse-DCF; grid in `config.valuation_assumptions`. **Dossier engine (P3, `analyst/dossier.py` + `law1.py` + `serialize.py`):** 8-stage / 5-clause synthesis over the serialized pack (the block IS the grounding whitelist), config-driven model (`config.synthesis_model`, seeded `claude-sonnet-4-6`; the seeder refuses to overwrite a changed row), ONE bounded repair pass (draft + violations fed back, minimal-edit instruction — a regenerate-style repair *introduced* violations), then three gates pre-store: Law-1 lint (instruction shapes; `analyst/law1.py`), numeric grounding vs the block (stage refs + SEC form names masked as nomenclature), and verdict-consistency (Graham MoS must be the valuation's own figure or null when n/m; ruin-list strings run the Law-1 patterns). Store to `analyses` with frozen pack+valuation, then `--send` delivers via Telegram (plain text, 4096-split). **P4:** `/analyze <TICKER>` in the webhook (shape-gate, BRK.B→BRK-B, instant ack) → `workflow_dispatch` on `analyze.yml`. Live rows: TSLA (EXPENSIVE / Mediocre@Premium / FRAGILE, MoS n/m) and GM (CHEAP 69.4% MoS / Mediocre@Discount / FRAGILE) — the §6 glamour/value disagreement. Known limitation: thousands-denominated filers (PLTR) push the model into unit conversion; the gate correctly blocks the store — prompt iteration continues. `analyst`/`sec_facts`/`config_read` are NON_DATA_SOURCES (dossier failures surface via the red run + alert, not §5).
...
- **Dossier runs (SPEND ~$0.30–0.60/run — Sonnet synthesis):** `python -m analyst.dossier TSLA --print-only` (synthesize + all three gates, NO store — the prompt-iteration loop); `python -m analyst.dossier TSLA` (store to `analyses` on gate pass); `--send` additionally delivers to Telegram (the analyze.yml step).
- **Seeds:** `python -m ingestion.seed_corporate_actions` (idempotent); `python -m ingestion.seed_config` (bootstrap only — refuses on a live config; `--missing-only` adds absent keys); `python -m analyst.seed_peers` (single-key `peer_overrides` upsert + peer instruments rows — never a full config seed); `python -m analyst.seed_synthesis_model` (single-key; refuses to overwrite a deliberately changed row).
- **Domain CLI (spec'd, not yet implemented):** `seed-calendar --year YYYY` — guided annual seeding of FOMC/CPI/NFP dates.
- **Telegram commands:** `/pulse` `/analyze` `/book` `/journal` `/felt` `/skip` `/health` `/override`.
