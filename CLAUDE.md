...
**Current state (as of 2026-07-01 — verify against `git log`, not this prose):**
...
- **Phase 1 (digest + bot) — done.** Full pipeline (`digest/`: fetch → URL dedup → Haiku sentiment → local indicators → bundle → Sonnet 5-clause synthesis → numeric grounding gate (`digest/grounding.py`, Law 2 enforced pre-store/pre-send) → store + frozen `bundle_json` → Telegram), plus `--dry-run`. The Vercel webhook (`api/webhook.py`) with fail-closed secret + chat-id auth (`bot/webhook_auth.py`); bot handlers `/book /journal /skip /health /override /pulse /felt` (`bot/handlers.py`); GitHub Actions `daily.yml`, `digest.yml`, `event_filter.yml`.
