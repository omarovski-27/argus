...
- **Phase 5 (analyst) — data + data-pack layers built, dossier engine not.** SEC company-facts mapper (11 concepts incl. capex, `ingestion/sec_facts.py`, now multi-issuer with a symbol↔CIK pairing gate + fill-gap tags for ASC-606/CoGS filers) → `fundamentals`/`fundamentals_latest`; split ledger + filed-date split reads (`quant/splits.py`); read-time metrics (`quant/metrics.py`). **Data-pack layer (`analyst/`):** ticker→CIK (`cik.py`), peer discovery (config `peer_overrides` > Finnhub; TSLA→[GM,F,RIVN,LCID] seeded, fundamentals ingested), EDGAR 10-K/DEF 14A section extraction (`filings.py`, TOC/cross-ref/page-header-proof), yfinance Stage-8 estimates (`estimates.py`), self-healing any-ticker ingestion (`ingest.py`), frozen-pack builder (`data_pack.py`, ~295 KB for TSLA). **Valuation engine (P2, `quant/valuation.py`):** owner-earnings base (basis-labeled; TSLA 2018+ falls back to OCF−capex — its D&A hides in a custom `tsla:` tag, correct reduced depth), 3-scenario discounted range, sensitivity + biggest mover, base-rate flag, bear-weighted MoS, closed-form reverse-DCF; grid in `config.valuation_assumptions` (range- and monotonicity-validated, fail-loud, seeded via `python -m analyst.seed_valuation`); 12th concept `depreciation_amortization`. `analyses` table is live (applied 2026-07-05). Not built: dossier synthesis + Law-1 lint (P3), `/analyze` wiring (P4).
...
- **Seeds:** `python -m ingestion.seed_corporate_actions` (idempotent); `python -m ingestion.seed_config` (bootstrap only — refuses on a live config; `--missing-only` adds absent keys); `python -m analyst.seed_peers` (single-key `peer_overrides` upsert + peer instruments rows — never a full config seed).
