...
1. **Information, never instruction.** No Argus output may say buy/sell/safe-to-trade or recommend timing. *Amended 2026-07-12 (Omar): Analyst dossiers render a framework-derived bottom-line rating (see analyst-module §2). Timing and sizing remain excluded. All other surfaces unchanged.* *Amended 2026-07-13 (Omar): Argus renders a pre-registered experimental timing signal ('Signal Lab') with a mandatory evidence label and live track record. Signals are hypotheses under test, never unlabeled instructions. Promotion of any signal into the trading strategy requires a PASS verdict at its pre-registered gates AND an explicit recorded decision by Omar.*
...
| 2b | Sleeve | **sleeve_pct = 20%** (locked config parameter). Share count is derived, never hardcoded: at sleeve entry `sleeve_shares = floor(sleeve_pct × live_portfolio_value ÷ TSLA_price)`, where `live_portfolio_value` = Flex positions market_value + cash. Frozen as the registered unit through the 50-trade verdict; re-derived only at a phase gate. Phase ladder: A 20% → B 30–40% → C 50%+, each gate = checkpoint pass. (The old '$35K → 17 shares' was a one-time snapshot illustration, not a live basis.) |
...
- **Sleeve:** `sleeve_pct` = 20% of live portfolio value (Flex positions market_value + cash) — not a fixed dollar figure. `sleeve_shares` is derived at sleeve entry — `floor(sleeve_pct × portfolio_value ÷ TSLA_price)` — then frozen as the registered unit through the 50-trade verdict; re-derived only at a phase gate. Sleeve only ever round-trips; core + all DCA deposits untouchable (Law 5). Corporate Actions auto-adjusts the frozen share count on splits.
